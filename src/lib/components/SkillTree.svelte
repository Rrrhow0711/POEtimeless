<script lang="ts">
  ...
  import { zhTW as tZh } from '../i18n/zh-TW';
  import { translateAlternatePassiveSkillName, translatePassiveSkillName } from '../i18n/game-zh-TW';
  ...
  $: render = (({ context, width, height }) => {
    ...
    if (hoveredNode) {
      let nodeName = translatePassiveSkillName(hoveredNode.name);
      let nodeStats: { text: string; special: boolean }[] = (hoveredNode.stats || []).map((s) => ({
        text: translatePassiveSkillName(s),
        special: false
      }));
        ...
        if (hoveredNode.skill && seed && selectedJewel && selectedConqueror) {
          ...
          if (result) {
            if ('AlternatePassiveSkill' in result && result.AlternatePassiveSkill) {
              nodeStats = [];
              nodeName = translateAlternatePassiveSkillName(result.AlternatePassiveSkill.Name);
              ...
              if ('StatsKeys' in result.AlternatePassiveSkill) {
                result.AlternatePassiveSkill.StatsKeys.forEach((statId, i) => {
                  const stat = data.GetStatByIndex(statId);
                    ...
                    if (translation) {
                      nodeStats.push({
                        text: translatePassiveSkillName(formatStats(translation, result.StatRolls[i]) || stat.ID),
                        special: true
                      });
                    }
                });
              ...
              result.AlternatePassiveAdditionInformations.forEach((info) => {
                if ('StatsKeys' in info.AlternatePassiveAddition) {
                  info.AlternatePassiveAddition.StatsKeys.forEach((statId, i) => {
                    ...
                    if (translation) {
                      nodeStats.push({
                        text: translatePassiveSkillName(formatStats(translation, info.StatRolls[i]) || stat.ID),
                        special: true
                      });
                    }
                  });
