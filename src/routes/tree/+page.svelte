<script lang="ts">
  ...
  $: statItems = availableStats
    .map((statId) => {
      ...
      return {
        label: translatePassiveSkillName(translateStat(id)),
        value: id
      };
    })
  ...
  const combineResults = (
    ...
    return Object.keys(mappedStats).map((statID) => {
      const translated = translatePassiveSkillName(translateStat(parseInt(statID)));
      return {
  ...
  const onPaste = (event: ClipboardEvent) => {
    ...
    const jewel = jewels.find((j) => j.label === lines[2]) || jewels.find((j) => j.rawLabel === lines[2]);
    if (!jewel) {
      return;
    }
    ...
    for (let i = 10; i < lines.length; i++) {
      conqueror = Object.keys(data.TimelessJewelConquerors[jewel.value]).find(
        (k) => lines[i].indexOf(k) >= 0 || lines[i].indexOf(translateConquerorName(k)) >= 0
      );
      if (conqueror) {
    ...
    seed = newSeed;
    selectedJewel = jewel;
    selectedConqueror = { label: translateConquerorName(conqueror), value: conqueror };
    updateUrl();
  };
  ...
  const platforms = [
  {
    ...
    value: 'Xbox',
    label: t.platformXbox
  }, {
    value: 'Playstation',
    label: t.platformPlaystation
  }
  ];
  ...
  const getLeagues = async () => {
    ...
    leagues = responseJson.map((l: { name: string }) => ({ value: l.name, label: translateLeagueName(l.name) }));
...
<SkillTree
  ...
  {#if !collapsed}
    <div
      ...
      <div class="p-4 max-h-screen flex flex-col">
        <div class="flex flex-row justify-between mb-2">
          ...
          {#if searchResults}
            <div class="flex flex-row gap-2">
              {#if results}
                <Select items={leagues} bind:value={league} on:change={updateUrl} clearable={false} placeholder={t.selectPrompt} />
                <Select items={platforms} bind:value={platform} on:change={updateUrl} clearable={false} placeholder={t.selectPrompt} />
                <button
        ...
        {#if !results}
          <Select items={jewels} bind:value={selectedJewel} on:change={changeJewel} placeholder={t.selectPrompt} />

          {#if selectedJewel}
            <div class="mt-4">
              <h3 class="mb-2">{t.conqueror}</h3>
              <Select items={conquerors} bind:value={selectedConqueror} on:change={updateUrl} placeholder={t.selectPrompt} />
            </div>
            ...
            {#if selectedConqueror && Object.keys(data.TimelessJewelConquerors[selectedJewel.value]).indexOf(selectedConqueror.value) >= 0}
              ...
              {#if mode === 'seed'}
                ...
                {#if seed >= data.TimelessJewelSeedRanges[selectedJewel.value].Min && seed <= data.TimelessJewelSeedRanges[selectedJewel.value].Max}
                  <div class="flex flex-row mt-4 items-end">
                    <div class="flex-grow">
                      <h3 class="mb-2">{t.sortOrder}</h3>
                      <Select items={sortResults} bind:value={sortOrder} placeholder={t.selectPrompt} />
                    </div>
                    <div class="ml-2">
                      <button
                  ...
                  {#if !split}
                    <ul class="mt-4 overflow-auto" class:rainbow={colored}>
                      {#each sortCombined(combineResults(seedResults, colored, 'all'), sortOrder.value) as r}
                        <li>
                          <button type="button" class="cursor-pointer w-full text-left" on:click={() => highlight(seed, r.passives)}>
              ...
              {:else if mode === 'stats'}
                <div class="mt-4">
                  <h3 class="mb-2">{t.addStat}</h3>
                  <Select items={statItems} on:change={selectStat} bind:this={statSelector} placeholder={t.selectPrompt} />
                </div>
                {#if Object.keys(selectedStats).length > 0}
                    ...
                    {#each Object.keys(selectedStats) as s}
                      <div class="mb-4 flex flex-row items-start flex-col border-neutral-100/40 border-b pb-4">
                        <div>
                          ...
                          <span>{translatePassiveSkillName(translateStat(selectedStats[s].id))}</span>
                        </div>
                        <div class="mt-2 flex flex-row">
                          <div class="mr-4 flex flex-row items-center">
                            <div class="mr-2">{t.min}:</div>
                            <input type="number" min="0" bind:value={selectedStats[s].min} />
                          </div>
                          <div class="flex flex-row items-center">
                            <div class="mr-2">{t.weight}:</div>
                  ...
                  <div class="flex flex-col mt-2">
                    <div class="flex flex-row items-center">
                      <div class="mr-2 min-w-fit">{t.minTotalWeight}:</div>
                  ...
                  <div class="flex flex-col mt-4">
                    ...
                    <div class="flex flex-row mt-2">
                      <button
                        ...
                        disabled={searching}>
                        {#if searching}
                          {t.searchProgress(currentSeed, data.TimelessJewelSeedRanges[selectedJewel.value].Max)}
                        {:else}
                          {t.search}
                        {/if}
                      </button>
              ...
              {#if !circledNode}
                <h2 class="mt-4">{t.clickJewelSocket}</h2>
              {/if}
            {/if}
          {/if}
        {/if}
  ...
  <div class="text-orange-500 absolute bottom-0 right-0 m-2 flex gap-4">
    <a href="/THIRD_PARTY_NOTICES.txt" target="_blank" rel="noopener">{t.licenseNotice}</a>
